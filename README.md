...
1. **Make changes** by editing content and/or adding resources.
    ...
    Content pages are written in Markdown, see the [basic syntax guide](https://www.markdownguide.org/basic-syntax/) for a quick overview. **Note** It is not mandatory to use Markdown, it is also fine to just write plain HTML. Within limits is is even possible to mix the two in one file (not recommended though).
